...
### 3-4. compact 범용 팝업 — 40초 자동 닫힘 (필수 · 2026-08-24~)

| 항목 | 정본 |
|------|------|
| 기본 동작 | `showArcAlert` · `presentArcOverlayAlert` · `waveResult` · `reward` · `levelUp` → **40초 후 자동 dismiss** |
| 상수 | `ARC_OVERLAY_DEFAULT_AUTO_DISMISS_MS` (`overlayAlertContract.ts`) |
| Host | `ArcOverlayHost` — compact kind `entry.autoDismissMs` timer |
| 단버튼 | 자동 닫힘 = 확인과 동일 (전투 패배 귀환 등) |
| 예외 | `autoDismissMs: 0` — 수동 확인만 (파괴적 2단 등, 주석 필수) |

일일 채굴 한도·전투 패배 결과창·Heavy UI preflight·시설 거부 등 **향후 compact 팝업 추가 시 별도 타이머 코드 불필요** — 해당 kind present만.
