---
description: Arcfire Online 아키텍처 마스터 스펙 v4.0 — 헌법·STAGE·15대 금지·Daily Ops 통합
alwaysApply: true
---

# 🚀 아크파이어 온라인 — React Native 모바일게임 프로젝트 아키텍처 마스터 스펙 v4.0 (무손실 완전판)

> **문서 버전**: v4.0 (Single-Player Ultimate & ArcCore Native - Unabridged Integration)
> **최종 업데이트**: 2026-06-14 
...
> **구현 세부 인덱스**: `AGENTS.md` (테이블·행성 허브·Skia·요격 등 운영 계약 요약)

---

## 🔄 정밀 검사 리포트: 파편화 문서 v4.0 완전 병합 내역

| 해결된 충돌/오류 | v4.0 해결 방안 (반영 완료) |
|------------------|----------------------------|
...
| **[v4.0 스펙 통합] 파편화 문서 100% 무손실 병합** | 전함 분류 표준(v3.1), 경제/무역(v3.1), 통합 레벨링 자동화(v3.1), 궤도 트래픽 및 행성 점유(v3.1) 스펙을 본 마스터 스펙 내 관련 섹션으로 원문 유실 없이 완벽 병합. |
...
Firestore 사용 범위 (실 플레이어 데이터만 단발성 동기화):
  ...
  ✅ BM 구매 및 퍼널 데이터
  ✅ arcfire_player_v1/{uid}/planet_holds (행성 점유 단발성 병합 업데이트)
  
  ❌ 실시간 타유저 동기화 (onSnapshot) → 절대 금지
  ❌ 유저 실시간 XY 좌표 업데이트 → 절대 금지
  ❌ AI 트래픽/가상 유저 상태 데이터 Firestore 저장 → 절대 금지
  ❌ ownership_claim/claim 트랜잭션 및 실시간 분쟁 → 절대 금지
```
...
4. 사용자 인증 및 STAGE 진입
5. 스테이지 구조 및 메모리 예산
6. 메인 스테이지 허브 (Stage 1) 및 궤도 트래픽 / 행성 로컬 점유
7. 게임플레이 루프: 출격·이동·전투 (Stage 2~3) 및 전함 분류 표준
8. 메모리 관리 핵심 설계 (누수 방지 패턴)
9. 렌더링 파이프라인 (Skia)
10. 아크코어 일일 운영 (Daily Ops), 경제/무역 및 통합 레벨링
...
### 6-2. 궤도 트래픽 프레즌스 (Hub Orbit Traffic Presence)
* **전면 폐기 사항**: 타 유저 접속 상태를 표시하던 `onSnapshot` 기반 Shared Presence 시스템, 4초 하트비트, 실시간 전투 공유(PVP 연동) 전면 폐기.
* **오프라인 AI 연출**: 메인스테이지 주변을 배회하는 NPC 트래픽(가상 전함)은 `AiNpcSubCore`가 전담. Firestore 연결 없이 `tables/content/npc_ai_ships.csv` 및 `npc_ai_captains.csv` 기반으로 로컬 생성.
* **생성 제약**: STAGE 1 메모리 예산(200MB)에 맞춰 동시 생성 개수 최대 5대 제한. 15초 내외 주기로 난입(Spawn) 및 이탈(Despawn).
* **데이터 구조**: `planetMemoCache/hub_traffic/{npcUid}`에 휘발성으로 저장되며, 행성 이탈이나 전투 진입 시 STAGE 메모리와 함께 즉각 해제(`planetSessionRegistry` dispose) 처리. info 패널에는 세계관 몰입을 위해 `‹AI› 닉네임` (또는 `‹P›`) 표시.

### 6-3. 행성 로컬 점유 시스템 (Planet Local Ownership)
* **전면 폐기 사항**: 타 실제 유저와의 소유권 분쟁, 경매, `ownership_claim/claim` Firestore `runTransaction` 및 실시간 `onSnapshot` 동기화 전면 금지.
* **싱글 샌드박스 시스템**: AI 팩션(무역 연합, 제국, 해적 등)이 선점한 행성의 무역소 소유권 증서를 플레이어가 구매하여 로컬에서 자신의 영토로 편입하는 구조.
* **UI 연출**: 행성 허브 및 무역소 진입 시 클랜 플레이트에 `(팩션 점유)` 혹은 획득 시 본인/로컬 클랜명 `(소유중)`으로 표시. 소유 시 무역소 수수료 인하 등 로컬 혜택 적용.
* **데이터 구조**: `arcfire_player_v1/{uid}/planet_holds` 프로필 문서 내에만 단발성으로 병합 업데이트됨. 트랜잭션 충돌 관리가 불필요하며 본인 프로필 업데이트 권한만 요구.
...
### 7-2. 전함 분류 표준 (Capital Ship Classification)
플레이어 및 AI 적 전함은 즉각적으로 **1순위 인지**가 가능해야 한다. (기존 PvP 암시 식별자 `npc_mock_pvp_ship`은 `npc_mock_ai_ship`으로 명칭 완전 순화)
* **인지 표기**: `Tier N · 함급(한·영) · Loadout형 · 전투거리 · 역할 한 줄`
...
* **Loadout vs Archetype**: UI 및 인지는 함급을 최우선으로 하고 Loadout은 2축으로 둔다. 전투 수학 연산(STAGE 3 로컬 물리 시뮬레이션)은 `capitalShipArchetype`을 통해 `ShipPerformanceCalculator`로 전달.
* **함선 폴백 로직 (Table-First)**:
...
4. **Strategic Beacon Engine**: 유저가 유인 비콘 등을 설치하면 `AiNpcSubCore`가 트래픽을 해당 좌표로 재정렬하고 `postStepRef` 콜백을 일일이 변경한다.
...
### 10-2. 경제 시뮬레이션 및 무역 생태계 (Economy SIM & Trade)
* **설계 비전**: 행성별 레벨 밴드 진열, 플레이어 성장 레벨 게이트, 행성 간 SKU 중복 및 생산·소비·거리 차익이 어우러진 RPG 하이브리드 무역.
* **실시간 시장 탄력 제거**: 플레이어의 대량 매매에 즉각적인 가격 폭등/폭락이 발생하지 않도록 **시장 탄력 없음(`price_elasticity=0`)**을 아키텍처 스펙으로 공식 확정. 
* **파이프라인 흐름**: 
  1. `npm run sim:economy` (Macro SIM)
  2. `economySimOverlayDelta.ts` 델타 생성
  3. 매일 12:00 KST 앱 기동 시 `runArcCoreDailyOpsBatch` 내부의 `runMarketPricePass`에서 로컬에 1회 누적 반영.
* **무역소 인덱스**: `tradePortCatalogPolicy.ts`, `tradeRouteCommercePolicy.ts` 기반 Map O(1) 조회 수행. 총 21행성 중 17곳이 무역소 보유 (군사 전초, 고대 유적, 심연 관문, 엔드 왕좌 제외).
* **KPI**: 싱글 샌드박스 밸런스 유지를 위해 F2P < Dolphin < Whale 순위를 강제. Whale/F2P power 비율 기준 < 5 (ok), 5~8 (warn), ≥ 8 (critical).

### 10-3. 통합 레벨링 자동화 (Integrated Leveling Automation)
* **실시간 재배치 금지**: 기존의 4초 디바운스 및 90초 주기 실시간 overlay 갱신 구조를 완전 폐기. 전투 도중 혹은 직후에 적의 HP 배율이 실시간으로 변동되는 아키텍처는 절대 금지.
* **로컬 텔레메트리**: STAGE 3 전투 종료 시 `recordMatchSummary`를 통해 로컬 `AsyncStorage`에만 로깅 처리 (멀티플레이 데이터 없음).
* **일 1회 보정 로직**: 12:00 KST Daily Batch 구동 시, 누적된 텔레메트리(평균 교전시간)와 `targetEngageSec(32)`를 비교 분석하여 `planetCoreRuntimeStore.globalMultipliers`의 `globalEngageHpMul` 값을 1회 갱신. 보정폭은 ±0.025~0.05 이며, 상/하한 캡 0.7~1.3 내에서만 적용.
...
* **정기 감사**: `npm run audit:daily`, `npm run audit:memory`, `npm run audit:balance`, `npm run audit:planet-combat-balance`
...
├── arcCore/                   ← 아크코어 근원 마스터
│   ├── subcores/
│   │   ├── ArcCoreDailyOpsSubCore.ts   ← [핵심] 일 1회 배치 (12:00) 경제/레벨링 통합
│   │   ├── AiNpcSubCore.ts             ← [핵심] 궤도 연출 (오프라인 트래픽 시뮬)
│   │   └── AiEconomySubCore.ts         
│   └── schedule/
...
│   └── planetMemoCache.ts              ← 행성 단위 휘발성 캐시 (궤도 트래픽 포함)
├── hooks/
...
> **Agent & Human Developer Rules**: 코드를 수정하는 모든 에이전트는 아래 16개 항목을 **최우선 시스템 헌법**으로 준수해야 한다. 위반 시 시스템 붕괴로 간주한다.
...
8. **[기획/AABS]** AABS 보정 시 1회 5%, 누적 0.7~1.3 상한 캡 초과 로직 작성 금지. 고빈도 실시간 HP 배율 갱신 또한 절대 금지.
...
10. **[아키텍처]** 멀티플레이/채널 관리를 위한 Firestore `onSnapshot` 통신, 유저 간 실시간 좌표 동기화, `hub_peers` 구독 로직 전면 절대 금지.
11. **[크래시 방지]** Stage 3 전투 종료 후 Skia Canvas `dispose()`가 완전 실행되기 전에 BM 결제/부활 모달 진입 절대 금지 (OOM 유발).
12. **[아키텍처/가상유저]** `aiVirtualPlayerStore` 같은 가상 유저 전용 독자 스토어 임의 재생성 전면 금지. 생태계 트래픽은 오프라인 `AiNpcSubCore`로 병합 통제된다.
13. **[렌더링]** 전투 시각화를 위한 SVG 레거시 경로 활성화 및 이중 구현 금지. `PlanetEdenRaidOrbitSkiaCombat` 단일 경로만 유지.
14. **[운영/경제]** AABS 및 경제 정책 재정렬, 실시간 시장 가격 변동(`price_elasticity`)을 고빈도로 실행 금지. 반드시 `runArcCoreDailyOpsBatch()`를 통해 하루 1회 12:00에만 일괄 처리.
15. **[UI/알림]** 에이전트 임의로 Alert/Modal 셸을 산재시키지 마라. 알림은 반드시 `ArcOverlayHost` 파이프라인(`showArcAlert` / `useArcBlockingOverlay`)을 경유하라.
16. **[DB/점유]** 타 유저와의 행성 소유권 입찰 경쟁, DB 트랜잭션(`ownership_claim/claim`) 절대 금지. 행성 점유는 프로필 단발성 병합 업데이트 기반의 로컬 시스템만 지원.
...
- [x] 모든 `onSnapshot` 쿼리 1회성 교체 및 `hub_peers` 완전 제거
- [x] `ownership_claim` 실시간 분쟁 트랜잭션 폐기

### P1 — 구조 보강 (성능 확보)
- [x] `useStageMemory` 훅 STAGE 1,2,3 적용 완료
- [x] `INFO_DISTANCE_SORT_INTERVAL_MS = 5000` 간격 스로틀링 적용
- [x] `runArcCoreDailyOpsBatch()` 12시 스케줄링 등록 (경제, AABS 통합)
...
- [x] `audit:balance`, `audit:memory`, `audit:planet-combat-balance` CI 연동
...
graph TD
    ...
    subgraph STAGE_1_Hub [Stage 1: Offline Sandbox Hub]
        MAIN_STAGE --> SKIA_LAYER[Skia: AiNpcSubCore 궤도 트래픽 최대 5대 + Nebula]
        MAIN_STAGE --> RN_ANIM[RN Transform: NPC 마커]
        MAIN_STAGE --> SORT_INFO[Distance Sort: 5000ms Throttle]
        MAIN_STAGE --> GOVERNOR[Governor System: Local Planet Claim]
    end
...
Navigation.replace()" --> GALAXY_MAP[Galaxy Map - Stage 2]
    ...
    %% 전투 종료 로깅, 해제, BM
    WIN_LOSE --> LOG_TELEMETRY[로컬 Telemetry 로깅: recordMatchSummary]
    LOG_TELEMETRY --> DISPOSE_COMBAT[Dispose: Skia + rAF 해제 완전 완료] --> CHK_SHOP{상점/부활 호출?}
    ...
    subgraph ArcCore_Daily_Batch [ArcCore Daily Ops Batch 12:00]
        OBS[Observe: 24h 로컬 관측 누적] --> ANALYZE[Analyze: AABS 검토, sim:economy Delta]
        ANALYZE --> EXECUTE[Execute: GlobalMultiplier 및 MarketPrice 1회 갱신]
    end
...
---
**Arcfire Architecture Master Spec v4.0 Final Unabridged — END**
