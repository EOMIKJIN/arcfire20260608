...
- 바걸 `016`의 마젠타·시안 네온 라운지·샷잔 열을 **함장·적·시드** 배경으로 사용 (함장·적·시드는 개인숙소·전함 내부·기지 복도·컴퓨터실)
- `npc_cpt_bar_ret_*` 바 주인은 이 금지의 대상이 아님 — §4-A
- 군복은 두고 장비·훈장만 폭주시키는 것

고유 네임드(스텔라 가운, 미아 앞치마 등)는 **군복 풀이 아니다**. 함장 증설과 섞지 않는다.

### 3-4. 인간형 기본 · 하이브리드 최소화 (2026-09-26 · 대표님)

생성·재생성 초상은 **자연스러운 인간 얼굴**(눈·코·입·피부)이 기본이다.

| 설정 | 초상 |
|------|------|
| 일반 인간 함장 | 전면 인간형. 호흡기·이마 바이저·의수·파워아머는 **얼굴이 보이면** 허용 |
| 기계·로봇·고대·창조자 | **하이브리드만** — 인간 얼굴·머리가 우선. 기계 암시는 옷깃 자수·관자놀이 표식 등 **최소** |
| 금지 | 전신 로봇 · 무표정 로봇 흉상 · 빈 마스크 · 홀로그램 실루엣 · 마네킹 · 외계 피부 · 촉수 머리 · 시계 인형 |

정본 생성 에이전트: `tools/content-tables/_PORTRAIT_GEN_AGENT.md`

### 3-5. 성별 (2026-09-26 · 대표님)

| `profileKo` | 초상 |
|-------------|------|
| `여성` · `그녀` · `여자` | 여성으로 읽혀야 함 |
| `남성` · `남자` | 남성으로 읽혀야 함 |
| 성별 단어 없음 · `개체` · 고대 감시만 | **중성** (남녀 구분이 어려운 얼굴) |

다른 인물을 가리키는 `그녀`는 이 함장의 성별로 쓰지 않는다.
...
## 4-A. 바 주인 `npc_cpt_bar_ret_*` (2026-09-26 · 대표님)

얼굴·나이는 기존 초상을 유지한다. **복장과 배경만** 바꾼다.

| 축 | 정본 |
|---|---|
| 복장 | 깔끔한 정장 · 고급 바텐더 · 주인 의상 (조끼·와이셔츠·보타이/넥타이). 스텔리움 군복·견장·에기예트 **금지** |
| 배경 | **행성 고유** 술집 바 라운지 인테리어(카운터·병선반·라운지 좌석). 행성마다 색·재질이 달라야 함 |
| 금지 | 바걸 KEEP(`bar_att_char006`–`016`)의 마젠타·시안 네온+샷잔 열 복제 · 함선 숙소·복도·함교·컴퓨터실을 바 주인 배경으로 유지 |

---

## 5. 이후 작업 순서

고유 함장 1행 = `assets/images/npc/<captainId>.png` 1장. 단계표: `docs/NPC_PORTRAIT_CANON_GENERATION_PLAN.md`.  
스텔라·플레이어 3·바걸은 **덮어쓰지 않는다.**

1. 본 문서 §2–§3을 생성 프롬프트에 그대로 넣는다. 참조 이미지 = **016 + (005 또는 010) + (필요 시 얼굴 레퍼런스)**  
2. PNG를 **240×240**으로 `assets/images/npc/<captainId>.png` 저장  
