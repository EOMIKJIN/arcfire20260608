...
- **행성 핵심지표**: `planets.csv`의 `coreResource/corePopulation/coreDefense/coreTechnology/coreEnvironment`(0..100)는 **초기 시드**만. 변화하는 값은 **`planetCoreRuntimeStore`**(AsyncStorage, `arcfire_planet_core_runtime_v1`)가 정본이다. `AiPlanetsSubCore`는 행성 단위 틱·시뮬 진입점이며, UI는 `R,P,D,T,E` 5색 디지털 게이지(20%/칸)로 표시한다. **에너지는 별도 축이 아니라 R(Resource)에 통합**되어 있다.
...
- **광물·Resource(지표) 고지 — 되는 것 / 안 되는 것** (에이전트는 답변·구현 시 **한계를 숨기지 말 것**):
  ...
  - **코드 위치**: `src/world/mineralDepositModel.ts`, `src/world/computeGalaxyMineralUniverseStats.ts`, `src/arcCore/planetEnergy/runPlanetEnergyCorePass.ts`, `src/arcCore/subcores/AiPlanetsSubCore.ts`.
