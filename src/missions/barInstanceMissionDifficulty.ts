/**
 * 바 ArcCore 인스턴스 의뢰 — 난이도 측정·등급(EASY~EXPERT)·보상 스케일.
...
export type BarInstanceMissionDifficultyTier = 'easy' | 'normal' | 'hard' | 'expert';

export type BarInstanceDifficultyFactors = {
  baseDc: number;
...
export type BarInstanceDifficultyResult = {
  score: number;
  tier: BarInstanceMissionDifficultyTier;
  factors: BarInstanceDifficultyFactors;
};

export const BAR_INSTANCE_DIFFICULTY_REWARD_MUL: Record<
  BarInstanceMissionDifficultyTier,
...
export function resolveDifficultyTierFromScore(score: number): BarInstanceMissionDifficultyTier {
...
export function computeBarInstanceMissionDifficulty(
  template: Mission,
  ctx: BarInstancePlanetContext,
  offerPlanetId: string,
): BarInstanceDifficultyResult {
  ...
  const factors: BarInstanceDifficultyFactors = {
    baseDc,
    levelRequired,
    deliveryHopCount,
    routeZoneRisk,
...
export function resolveBarInstanceReputationRewardMul(reputationBonusPct: number): number {
  const pct = Math.max(0, Number(reputationBonusPct) || 0);
  return 1 + pct / 100;
}

export function scaleBarInstanceMissionRewards(
  base: MissionReward,
  tier: BarInstanceMissionDifficultyTier,
  reputationBonusPct = 0,
): MissionReward {
  const mul = BAR_INSTANCE_DIFFICULTY_REWARD_MUL[tier];
  const rep = resolveBarInstanceReputationRewardMul(reputationBonusPct);
  return {
    credits: Math.max(100, Math.round(base.credits * mul.credits * rep)),
    exp: Math.max(50, Math.round(base.exp * mul.exp * rep)),
    items: base.items ? [...base.items] : undefined,
    skillPointBonus: base.skillPointBonus,
  };
}

export function formatBarInstanceDifficultyTierLabel(
  tier: BarInstanceMissionDifficultyTier,
): string {
  switch (tier) {
    case 'easy':
