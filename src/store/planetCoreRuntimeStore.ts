...
export const usePlanetCoreRuntimeStore = create<PlanetCoreRuntimeState>((set, get) => ({
  ...
  bootstrapFromWorldAsync: async () => {
    ...
    if (!get().hydrated) {
      ...
      try {
        ...
        if (raw) {
          const parsed = JSON.parse(raw) as { byPlanetId?: Record<string, unknown> };
      ...
      set({ byPlanetId: next, hydrated: true });
      try {
        await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify({ byPlanetId: next }));
      } catch {
        /* ignore */
      }
      return;
  ...
  persistPlanetCoreRuntime: async () => {
    try {
      await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify({ byPlanetId: get().byPlanetId }));
