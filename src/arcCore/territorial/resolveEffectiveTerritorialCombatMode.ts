...
//   + 비중립 hold 단측 인접 graph 정합 (2026-08-02, crimson_base LogBox mismatch)
...
// R1b(신규): 비중립 + contestedZone + 단측만 인접이면 그 쪽 *_neutral — 동적 분쟁 템플릿(blue_red 등)이
//     실제로는 한쪽만 인접인데 CSV/템플릿 값을 그대로 반환해 runtimeGraph(inferTerritorialCombatModeFromGraph)
//     추론과 어긋나던 갭을 없앤다(crimson_base: RED만 인접인데 effective=blue_red로 남던 사례).
// R2/R3: NEUTRAL hold에서만 CSV combatMode를 런타임 supplyAdjacency로 덮어씀(한쪽만 인접·고립 폴백).
//        BLUE/RED hold(양쪽 아님)는 contestedZone 시 위 R1/R1b, 아니면 기존 CSV combatMode 그대로.
 ...
 * 비중립(BLUE/RED) hold — R1/R1b(2026-08-02 정리):
 ...
 *   배제되는 재발(오메가 스테이션 사례)을 막는다.
 *   contestedZone && 블루만 인접>0 → `blue_neutral` / 레드만 인접>0 → `red_neutral`
 *   (동적 분쟁 템플릿이 blue_red 등으로 고착돼 runtimeGraph 단측 추론과 어긋나던 갭 제거).
 *   그 외(양쪽 모두 0·고립, 또는 비분쟁지역)는 policy.combatMode 그대로.
 */
export function resolveEffectiveTerritorialCombatMode(
  ...
  if (contestedZone && blueCan) return 'blue_neutral';
  if (contestedZone && redCan) return 'red_neutral';
  return policyCombatMode;
}
