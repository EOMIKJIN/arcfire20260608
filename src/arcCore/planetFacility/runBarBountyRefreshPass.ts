// ============================================================
// v2.0 — 바 레벨 → 바운티 슬롯·갱신 주기 메타 동기화
// (바운티 보드 본체는 barBoardStore — 향후 bounty 슬롯 확장)
// ============================================================

import { resolveBarBountySlots, resolveBarRefreshIntervalHours } from '../balance/facilityBarLevelPolicy';
import { generateBarBountyBoard } from '../../game/bar/barBountyGenerator';
...
export type BarBountyRefreshPassResult = {
  ran: boolean;
  planetsRefreshed: number;
};

export function runBarBountyRefreshPass(nowMs = Date.now()): BarBountyRefreshPassResult {
  ...
  for (const [planetId, hold] of Object.entries(holds)) {
    ...
    const intervalH = resolveBarRefreshIntervalHours(detail.level);
    const intervalMs = intervalH * 3600 * 1000;
    const last = detail.lastBountyRefreshTimestamp ?? 0;
    const slots = resolveBarBountySlots(detail.level);
    ...
    writeFacilityModuleDetail(planetId, PLANET_DEV_MODULE_POPULATION_DOME, {
      ...
      bountyBoard: needsRefresh
        ? generateBarBountyBoard(planetId, detail.level, slots, nowMs, intervalH)
        : detail.bountyBoard,
      updatedAtMs: nowMs,
    });
    planetsRefreshed += 1;
