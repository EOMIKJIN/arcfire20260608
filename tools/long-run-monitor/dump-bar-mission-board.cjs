...
const barBoard = getJson('arcfire_bar_board_v1');
  ...
  JSON.stringify(
    {
      ...
      barFacility: currentPlanetId ? devBarLevel(currentPlanetId) : null,
      bountyBoard: currentPlanetId ? bountyBoard(currentPlanetId) : null,
      galacticNewsBoard: {
        loaded: Boolean(barBoard),
        visibleNotices: barBoard?.notices?.length ?? 0,
        recentTitles: (barBoard?.notices ?? []).slice(0, 5).map((n) => ({
          tag: n.tag,
          title: n.title,
        })),
      },
