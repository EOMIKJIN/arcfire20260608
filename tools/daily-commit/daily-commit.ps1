...
$doAudit = if ($RunAudit) { $true } else { [bool]$DailyCommitRunAudit }
...
Write-Host "[daily-commit] repo=$RepoRoot push=$($Push.IsPresent) audit=$($RunAudit.IsPresent)"

npm run daily:commit 2>&1 | Tee-Object -FilePath $logFile -Append
exit $LASTEXITCODE
