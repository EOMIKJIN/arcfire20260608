...
const ROUTING_CONTEXT = [
  ...
  '  - profile:mem:watch 상시 프로파일(스냅샷·[MEM_PROFILE]·retention diff).',
  '  - audit:memory:retention — 닫힌 STAGE survivor 탐지 → FAIL 시 김팀장 인계.',
  '  - 이상 탐지 → incident handoff → **김팀장 인계**(김경제는 자동조치·코드 수정 안 함).',
  '  - 정본: tools/long-run-monitor/logs/WATCH_README.md · tools/memory-profiler/README.md',
  '',
